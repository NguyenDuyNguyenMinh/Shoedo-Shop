    ...
    <main class="main-content">
      <div class="page-container">
        ...
        <div class="row mb-4 g-3">
          <div class="col-md">
            <div
              class="card text-white stats-mini bg-gradient-1"
        ...
        <div class="tab-content p-3 bg-white rounded-bottom shadow-sm">
          ...
          <div v-show="activeTab === 'pending'">
            <div
              v-if="filteredOrders.pending.length === 0"
            ...
            <div v-else class="row g-3">
              ...
              >
                <div class="card h-100 border-dark">
                  ...
                  >
                    <strong
                      >#HD{{ String(order.maHD).padStart(4, "0") }}</strong
                    >
                  ...
                  <div class="card-body">
                    ...
                    <div v-if="order.maQT" class="mb-2">
                      ...
                      <small
                        >#QT{{ String(order.maQT).padStart(4, "0") }} -
                        {{ order.tenQT || order.emailQT }}</small
                      >
                    </div>
          ...
          <div v-show="activeTab === 'delivering'">
            <div
              v-if="filteredOrders.delivering.length === 0"
              class="text-center p-5 text-muted"
            >
            ...
            <div v-else class="row g-3">
              <div
                v-for="order in filteredOrders.delivering"
                :key="order.maHD"
                class="col-12 col-md-6 col-lg-4"
              >
                <div class="card h-100 border-dark">
                  ...
                  <div class="card-footer bg-transparent">
                    <div class="d-flex gap-2 mb-2">
                      <button
                        ...
                        @click="showDeliveryFailedModal(order)"
                      >
                        <i class="fas fa-truck me-1"></i>Thất bại
                      </button>
                      <button
                        class="btn btn-success btn-sm flex-fill"
                        @click="deliverySuccess(order.maHD)"
                      >
          ...
          <div v-show="activeTab === 'completed'">
            <div
              v-if="filteredOrders.completed.length === 0"
              class="text-center p-5 text-muted"
            >
                ...
                <div class="card h-100 border-dark">
                  ...
                  <div class="card-body">
                    ...
                    <div class="mb-2">
                      ...
                      <strong>{{
                        formatDate(order.ngayDen) || "Chưa cập nhật"
                      }}</strong>
                    </div>
          ...
          <div v-show="activeTab === 'rejected'">
            <div
              v-if="filteredOrders.rejected.length === 0"
              class="text-center p-5 text-muted"
            >
            ...
            <div v-else class="row g-3">
              <div
                v-for="order in filteredOrders.rejected"
                :key="order.maHD"
                class="col-12 col-md-6 col-lg-4"
              >
                <div class="card h-100 border-dark">
          ...
          <div v-show="activeTab === 'error'">
            <div
              v-if="filteredOrders.error.length === 0"
              class="text-center p-5 text-muted"
            >
            ...
            <div v-else class="row g-3">
              <div
                v-for="order in filteredOrders.error"
                :key="order.maHD"
                class="col-12 col-md-6 col-lg-4"
              >
                <div class="card h-100 border-dark">
                  ...
                  <div class="card-footer bg-transparent">
                    <button
                      ...
                      @click="sendApologyEmail(order.maHD)"
                    >
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">
              Chi tiết đơn hàng #HD{{
                selectedOrder ? String(selectedOrder.maHD).padStart(4, "0") : ""
              }}
            </h5>
            <button
              type="button"
          ...
          <div class="modal-body">
            <div v-if="orderDetail">
                ...
                <div class="col-md-5">
                  ...
                  <div class="border p-3 rounded">
                    <div class="d-flex mb-2">
                      ...
                      <div>
                        #HD{{ String(orderDetail.maHD).padStart(4, "0") }}
                      </div>
                    </div>
                    <div class="d-flex mb-2">
                    ...
                    <div class="d-flex mb-2">
                      ...
                      <div>
                        <span
                          :class="
                            'badge ' +
                            getStatusBadgeClass(orderDetail.trangThai)
                          "
                        >
                    ...
                    <div class="d-flex">
                      ...
                      <div class="fw-bold text-primary">
                        {{ formatPrice(orderDetail.tongTien) }}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="mb-4">
                ...
                <div class="table-responsive">
                  <table class="table table-bordered">
                    ...
                    <tbody>
                      <tr
                        v-for="(item, index) in orderDetail.chiTiet"
                        :key="index"
                      >
                        <td>{{ index + 1 }}</td>
                        <td>{{ item.tenSP }}</td>
                        <td>
                          {{ item.tenMau }} - Size
                          {{ item.coGiay === 0 ? "Free" : item.coGiay }}
                        </td>
                        <td>{{ item.soLuong }}</td>
              ...
              <div v-if="orderDetail.maQT" class="mb-3">
                ...
                <div class="border p-3 rounded">
                  <div class="row">
                    <div class="col-md-4">
                      <span
                        ><strong>Mã NV:</strong> #QT{{
                          String(orderDetail.maQT).padStart(4, "0")
                        }}</span
                      >
                    </div>
                    <div class="col-md-4">
                      <span
                        ><strong>Tên NV:</strong> {{ orderDetail.tenQT }}</span
                      >
                    </div>
                    <div class="col-md-4">
                      <span
                        ><strong>Email:</strong> {{ orderDetail.emailQT }}</span
                      >
                    </div>
                  </div>
                </div>
              ...
              <div
                v-if="orderDetail.ghiChu"
                class="mt-3 p-3 border rounded"
                :class="getNoteClass(orderDetail.trangThai)"
              >
          ...
          <div class="modal-body">
            <p>
              ...
              <strong
                >#HD{{
                  orderToReject
                    ? String(orderToReject.maHD).padStart(4, "0")
                    : ""
                }}</strong
              >?
            </p>
    ...
    >
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>
              Đánh dấu giao hàng thất bại cho đơn
              <strong
                >#HD{{
                  orderToFail ? String(orderToFail.maHD).padStart(4, "0") : ""
                }}</strong
              >
            </p>
  ...
  setup() {
    ...
    const filteredOrders = computed(() => {
      ...
      const filterFn = (order) => {
        if (keyword) {
          const maHDStr = `HD${String(order.maHD).padStart(
            4,
            "0"
          )}`.toLowerCase();
    ...
    const confirmOrder = async (orderId) => {
      if (
        !confirm("Xác nhận đơn hàng này? Số lượng sản phẩm sẽ bị trừ khỏi kho.")
      )
        return;
      processing.value = true;
      try {
        const response = await axios.post(
          `/api/employee/orders/${orderId}/confirm`
        );
        if (response.data.success) {
        ...
        error.value =
          err.response?.data?.message || "Lỗi khi xác nhận đơn hàng";
      } finally {
    ...
    const confirmRejectOrder = async () => {
      ...
      try {
        const response = await axios.post(
          `/api/employee/orders/${orderToReject.value.maHD}/failed`,
          {
    ...
    const deliverySuccess = async (orderId) => {
      ...
      try {
        const response = await axios.post(
          `/api/employee/orders/${orderId}/delivery-success`
        );
        if (response.data.success) {
          message.value = response.data.message;
          await loadOrders();
        ...
        error.value =
          ...
          "Lỗi khi cập nhật giao hàng thành công";
      } finally {
        processing.value = false;
      }
    };
    ...
    const confirmDeliveryFailed = async () => {
      ...
      try {
        const response = await axios.post(
          `/api/employee/orders/${orderToFail.value.maHD}/delivery-failed`,
          {
        ...
        error.value =
          err.response?.data?.message || "Lỗi khi cập nhật giao hàng thất bại";
      } finally {
        processing.value = false;
      }
    };
    ...
    const sendApologyEmail = async (orderId) => {
      if (
        !confirm("Đã xử lý xong thông tin báo lỗi? Gửi email xin lỗi kèm PDF.")
      )
        return;
      processing.value = true;
      try {
        const response = await axios.post(
          `/api/employee/orders/${orderId}/send-apology-email`
        );
        if (response.data.success) {
          message.value = response.data.message;
          await loadOrders();
