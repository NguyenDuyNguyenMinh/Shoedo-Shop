...
public interface ThongKeDAO extends JpaRepository<HoaDon, Integer> {
    ...
    @Query(value = """
        SELECT 
            dbo.AllDates.DateValue AS ngay,
        ...
        FROM dbo.GetDatesBetween(:startDate, :endDate) dbo.AllDates
        LEFT JOIN HoaDon hd ON CAST(hd.NgayMua AS DATE) = dbo.AllDates.DateValue 
            AND hd.TrangThai = N'Hoàn tất'
        LEFT JOIN HoaDonCT hdct ON hd.MaHD = hdct.MaHD
        GROUP BY dbo.AllDates.DateValue
        ORDER BY dbo.AllDates.DateValue
        """, nativeQuery = true)
    ...
    @Query(value = """
        SELECT 
            dbo.AllMonths.MonthValue AS thang,
            ISNULL(SUM(hdct.DonGia * hdct.SoLuong), 0) AS doanhThu,
            COUNT(DISTINCT hd.MaHD) AS soDonHang,
            CASE 
                WHEN COUNT(DISTINCT hd.MaHD) > 0 
            ...
            END AS giaTriDonTB,
            SUM(hdct.SoLuong) AS soSP
        FROM dbo.GetMonthsBetween(:startDate, :endDate) dbo.AllMonths
        LEFT JOIN HoaDon hd ON FORMAT(hd.NgayMua, 'yyyy-MM') = dbo.AllMonths.MonthValue 
            AND hd.TrangThai = N'Hoàn tất'
        LEFT JOIN HoaDonCT hdct ON hd.MaHD = hdct.MaHD
        GROUP BY dbo.AllMonths.MonthValue
        ORDER BY dbo.AllMonths.MonthValue
        """, nativeQuery = true)
